				...
				if($type=='state'){ ?>
					<table class="table table-bordered secoverlay3 fixedTable">
						...
						<tr class="table-header">
							<th rowspan="2" width="21%" >State/ UT Name</th>
							<th colspan="3" class="text-center" >Districts Covered</th>
							<th colspan="5" class="text-center" >Biogas Plants Registered</th>
						</tr>
						
						<tr class="table-header">
							<th class="text-center" >Total No of Districts </th>
							<th class="text-center" >Number of Districts covered</th>
							<th class="text-center" >% Districts covered</th>
						...
						<?php
							...
							foreach($states as $state){ ?>
								<tr>
									<td>
										<a href="district-biogas?t=state&s=<?=$state['state_code'];?>&m=<?=@$_GET['m'];?>" > <?=$state['state_name'];?> </a>
									</td>
									
									<td class="text-center">
										<?php
											echo $state['noofdistricts'];
											$tDistricts+=$state['noofdistricts'];
										?>
									</td>
									
									<td class="text-center">
										<?php
											echo $nod=$state['districtCovered'];
											$tNoDistrictCovered+=$nod;
										?>
									</td>
									<td class="text-center"><?php echo round(($nod/$state['noofdistricts'])*100,1); ?> %</td>
						...
						<tr class="table-header">
							...
							<td class="text-center"><?=$tDistricts;?></td>
							<td class="text-center"><?=$tNoDistrictCovered;?></td>
							<td class="text-center"><?php echo round(($tNoDistrictCovered/$tDistricts)*100,1); ?> %</td>
...
$(document).ready(function(){
	...
	$("#ministry").on("change", function(){
		let m = $(this).val();
		if(m!=""){
			let url = 'https://gobardhan.co.in/state-biogas?t=state&m='+m;
	...
	$(".STbiogasPlantDetails").on("click", function(){
		...
		$.ajax({
            ...
            data:{stateCode:stateId,bgstatus:bgstatus,etype:17,m:mn},
