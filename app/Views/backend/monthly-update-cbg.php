	...
	<div class="row justify-content-center">
		<div class="col-sm-12">
			<h3 style="background-color: lightblue; padding: 7px;font-weight: bold;">
				Functionality Status of CBG Plants - <?=$_SESSION['name'];?>
			</h3>
		</div> 
								...
								<td>
									<ul class="list-group mupdate ">
										...
										<li class="list-group-item p-1 d-none" id="f<?=$project['project_id'];?>">
											...
											<input type="number" class="form-control mt-2 fsutilized" min="0" data-id="<?=($project['solid_feedstock_capacity']*$month_end->format('d'));?>" id="feedstock<?=$project['project_id'];?>" placeholder="Total feedstock utilized (in Tons) in <?php echo $month_ini->format('M-Y'); ?>" />
											<span class="text-danger fsutilized_err"></span>
											<input type="number" class="form-control mt-2 mxbiogas" min="0" data-id="<?=($project['gas_production_capacity']*$month_end->format('d'))?>" id="monthlyBiogasGen<?=$project['project_id'];?>" placeholder="Total CBG generated (in Tons) in <?php echo $month_ini->format('M-Y'); ?>" />
											<span class="text-danger mxbiogas_err"></span>
											<input type="number" class="form-control mt-2 mxbBioSlurry" min="0" data-id="<?=($project['bio_slurry_output']*$month_end->format('d'))?>" id="monthlyBioSlurry<?=$project['project_id'];?>" placeholder="Total bio slurry generated (in KL) in <?php echo $month_ini->format('M-Y'); ?>" />
											<span class="text-danger mxbBioSlurry_err"></span>
											<input type="number" class="form-control mt-2" min="0" maxlength="5" id="fom<?=$project['project_id'];?>" placeholder="Total FOM (in Tons) in <?php echo $month_ini->format('M-Y'); ?>" />
											<input type="number" class="form-control mt-2" min="0" maxlength="5" id="lfom<?=$project['project_id'];?>" placeholder="Total LFOM (in KL) in <?php echo $month_ini->format('M-Y'); ?>" />
										...
										<li class="list-group-item p-1 d-none" id="nonf<?=$project['project_id'];?>" >
											...
											<textarea type="text" class="form-control mb-2 " id="otherReason<?=$project['project_id'];?>" placeholder="Enter Reason (50 Words only)" ></textarea>
											...
											<input type="number" class="form-control mb-2" id="reqFunds<?=$project['project_id'];?>" placeholder="Estimated funds required to make it functional (in Crs.)" />
										</li>
...
$(".reporting").on("click", function(){
	...
	if(pid!="" && cstatus!=""){
		...
		if(cstatus==24){
			...
			if(feedstock!="" && monthlyBiogasGen!="" && monthlyBioSlurry!="" && rdate!=""&& fom!=""&& lfom!="" && docc!=false){
				...
				reportingData.append('fom',fom);
				reportingData.append('lfom',lfom);
				sendRequest=true;
