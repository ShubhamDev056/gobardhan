								...
								<td>
									<ul class="list-group mupdate ">
										...
										<li class="list-group-item p-1 d-none" id="f<?=$project['project_id'];?>">
											...
											<label class="mb-0">Total feedstock utilized (in Kgs) in <?php echo $month_ini->format('M-Y'); ?></label>
											<input type="number" class="form-control mb-2" min="0" maxlength="5" value="<?=$mmdetails['feedstock'];?>" id="feedstock<?=$project['project_id'];?>" placeholder="Total feedstock utilized (in Kgs) in <?php echo $month_ini->format('M-Y'); ?>" />
											<label class="mb-0">Total biogas generated (in Cum) in <?php echo $month_ini->format('M-Y'); ?></label>
											<input type="number" class="form-control mb-2" min="0" maxlength="5" value="<?=$mmdetails['biogas_generation'];?>" id="monthlyBiogasGen<?=$project['project_id'];?>" placeholder="Total biogas generated (in Cum) in <?php echo $month_ini->format('M-Y'); ?>" />
											<label class="mb-0">Total bio slurry generated (in L) in <?php echo $month_ini->format('M-Y'); ?></label>
											<input type="number" class="form-control mb-2" min="0" maxlength="5" value="<?=$mmdetails['bioslurry_generation'];?>" id="monthlyBioSlurry<?=$project['project_id'];?>" placeholder="Total bio slurry generated (in L) in <?php echo $month_ini->format('M-Y'); ?>" />
										...
										<li class="list-group-item p-1 d-none" id="nonf<?=$project['project_id'];?>" >
											...
											<select class="form-control check_multiselect mb-2" multiple placeholder="Select Reason" id="reason<?=$project['project_id'];?>" data-id="<?=$project['project_id'];?>" style="height:50px;" > 
												<?php
													$reason = explode(",",$mmdetails['reason']);
													$defunct_reasons = allOptions('defunct_reason');
													foreach($defunct_reasons as $key=>$defunct_reason){ ?> 
														<option value="<?=$key;?>" <?php if(in_array($key,$reason)){ echo "selected"; } ?> ><?=$defunct_reason;?></option>
													
													<?php
													}
												?>
											</select>
											<input type="text" class="form-control mb-2 d-none" value="<?=$mmdetails['other_reason'];?>" id="otherReason<?=$project['project_id'];?>" placeholder="Other Reason" />
											...
											<input type="number" class="form-control mb-2" id="reqFunds<?=$project['project_id'];?>" value="<?=$mmdetails['functional_amount'];?>" placeholder="Estimated funds required to make it functional (in Lakhs)" />
											<label class="mb-0">Source of funds required to make it functional</label>
											<input type="text" class="form-control" maxlength="50" id="reqSource<?=$project['project_id'];?>" value="<?=$mmdetails['functional_source'];?>" placeholder="Source of funds required to make it functional" />
										</li>
