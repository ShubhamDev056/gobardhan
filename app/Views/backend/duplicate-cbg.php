...
<div class="container mt-5">
	...
	<div class="row justify-content-center ">
		...
		<div class="col-sm-12 secoverlay">
			<h3 style="background-color: #d0e091; padding: 7px; font-weight: bold; margin-top: 30px;">District Wise CBG Plants Duplicate </h3>
		</div>
		<div class="col-lg-12 col-12 secoverlay2">
			<div class="table-responsive">
				<table class="table table-bordered secoverlay3">
					...
					<?php
						foreach($distWiseDuplicateCbgs as $distWiseDuplicateCbg){ ?>
							<tr>
								<td><?=$distWiseDuplicateCbg->state_name?></td>
								<td><?=$distWiseDuplicateCbg->district_name?></td>
								<td>
									<a href="javascript:void(0)" class="STbiogasPlantDetails" data-toggle="modal" data-target="#biogasPlantsModal" data-district="<?=$distWiseDuplicateCbg->dist;?>" >
										<?=$distWiseDuplicateCbg->noofPlants?>
									</a>
									<?php
										if($distWiseDuplicateCbg->pending>0){ ?>
...
$(document).ready(function(){
	...
	$(".STbiogasPlantDetails").on("click", function(){
		...
		$.ajax({
            ...
            type:"post",
            data:{districtCode:districtId,etype:18},
            success:function(res){
				...
				$.each(resps, function (key, val) {
					...
					pdetailsHtml+='<tr>\
						...
						<td>'+pstatus+' </td>\
						<td>\
							<a href="javascript:" class="btn btn-primary btn-sm approvebtn" data-id="'+val.project_id+'" style="color: white!important;font-weight: 400;" >Approve</a>\
							<a href="javascript:" class="btn btn-danger btn-sm deletebtn" data-id="'+val.project_id+'" style="color: white!important;font-weight: 400;" >Delete</a>\
		...
		if (confirm('Are you sure to approve?')) {
			...
			if(pid!=""){
				$.ajax({
					...
					success:function(res){
						//console.log(res);
						window.location.href="<?=base_url()?>/duplicate-cbg";
					}
				});
			}
		}
