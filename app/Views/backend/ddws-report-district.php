	...
	<div class="row justify-content-center">
		<div class="col-sm-12">
			<h3 style="background-color: lightblue; padding: 7px;font-weight: bold;margin-bottom: 10px;">
				District Wise DDWS Report - (<?=@$_REQUEST['s']?>) 
				<!--<a href="javascript:" id="export"> <i class="fa fa-download pull-right"></i> </a>-->
			</h3>
				...
				<div class="row">
					<div class="col-sm-5">
						<div class="form-group">
							<input type="date" name="fdate" class="form-control" placeholder="" value="<?=@$_REQUEST['fdate']?>" required />
						</div>
					</div>
					<div class="col-sm-5">
						<div class="form-group">
							<input type="date" name="tdate" class="form-control" placeholder="" value="<?=@$_REQUEST['tdate']?>" required />
						</div>
					</div>
					<div class="col-sm-2">
							...
							<tr>
								...
								<td class="text-center">
									...
									<?php
										...
										if($yetToStart>0){ ?>
											<a href="javascript:void(0)" class="STbiogasPlantDetails" data-district="<?=$state['district_code']?>" data-bgstatus="yettostart" >
												<?=$yetToStart;?>
											</a>
										<?php	
										}else{
								...
								<td class="text-center">
									...
									<?php
										...
										if($construction>0){ ?>
											<a href="javascript:void(0)" class="STbiogasPlantDetails" data-district="<?=$state['district_code']?>" data-bgstatus="underconstct" >
											<?=$construction;?>
											</a>
										<?php	
										}else{
								...
								<td class="text-center">
									...
									<?php
										...
										if($completed>0){ ?>
											<a href="javascript:void(0)" class="STbiogasPlantDetails" data-district="<?=$state['district_code']?>" data-bgstatus="completed" >
												<?=$completed;?>
											</a>
										<?php	
										}else{
								...
								<td class="text-center">
									...
									<?php
										...
										if($functional>0){ ?>
											<a href="javascript:void(0)" class="STbiogasPlantDetails" data-district="<?=$state['district_code']?>" data-bgstatus="functional" >
												<?=$functional;?>
											</a>
										<?php	
										}else{
...
$(document).ready(function(){
	...
	$(".STbiogasPlantDetails").on("click", function(){
		...
		$.ajax({
            ...
            data:{districtCode:districtCode,bgstatus:bgstatus,etype:17,m:mn,fdate:fdate,tdate:tdate},
